...
const stringIncludes = (str1, str2) => (str1.indexOf(str2) != -1? true: false)
...
console.log(stringLastIndexOf('january', 'a')) // 4
console.log(stringLastIndexOf('mellow', 'w')) // 5
console.log(stringLastIndexOf('jia', 'a')) // 2
...
function removeFromString(str, x, y){
   return str.substring(0, x) + str.substring(x+y)
...
function includes(coll, val, ind){
    let count = 0
    if (ind === undefined || coll instanceof Object){
        ind = 0
    }
    for(foo in coll){
        let obj = coll[foo]
        if (obj === val && count >= ind){
            return true
        }
        count++
    }
